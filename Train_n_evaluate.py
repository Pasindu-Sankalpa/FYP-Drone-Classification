...
class Train_n_evaluate_combined:
    ...
    def train_model(self, model, det_criterion, cls_criterion, optimizer, epochs, scheduler, alpha=0.5):
        ...
        for epoch in range(epochs):
            for phase in ["train", "validation"]:
                ...
                for audio, det_label, cls_label in tqdm(self.loaders[phase]):
                    audio, det_label, cls_label = (
    ...
    def evaluate_model(self, model, dataset, mode):
        ...
        for audio, det_label, cls_label in tqdm(self.loaders[dataset]):
            if mode:
                label = cls_label
            else:
                label = det_label
