...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.drone_5_image = customtkinter.CTkImage(Image.open(os.path.join(image_path, "drone5.jpg")), size=(200, 200))
        ...
        self.audio_entry.insert(0, "D:/FYP/Drone Classification/Pipeline/GUI_Combined/audio_capture/test.wav")
        self.audio_entry.grid(row=5, column=1, padx=(20, 40), pady=(10, 10), sticky="nsew", columnspan=2)
        ...
        self.radar_entry.insert(0, "C:/Program Files/ti/mmwave_studio_03_00_00_14/mmWaveStudio/PostProc/adc_data.bin")
        self.radar_entry.grid(row=6, column=1, padx=(20, 40), pady=(10, 10), sticky="nsew", columnspan=2)
        ...
        self.drone_4_label = customtkinter.CTkLabel(self.info_frame, text="DJI Phantom 4 Pro V2.0", image=self.drone_5_image, compound="top", font=customtkinter.CTkFont(size=15))
    ...
    def process_button_thread(self, new_file_name):
        loader = DataLoader(TestDataSet(new_file_name), batch_size=128, shuffle=True)

        drone_exist_val, drone_type_val = eval_comb(loader)

        self.drone_exist_val_label.configure(text=drone_exist_val)
        self.drone_type_val_label.configure(text=drone_type_val)
        self.home_process_button.configure(state="normal")
    
    def process_button_event(self):
        def find_next_file_name(base_folder, prefix):
            i = 1
            while True:
        ...
        self.home_process_button.configure(state="disabled")
        self.drone_exist_val_label.configure(text="Pending...")
        self.drone_type_val_label.configure(text="Pending...")
        ...
        print(path_1, path_2)
        new_file_name = find_next_file_name("./data", "test")
        shutil.copy(path_1, "./data/" + new_file_name + ".wav")
        shutil.copy(path_2, "./data/" + new_file_name + ".bin")

        # Create a thread
        thread = threading.Thread(target=self.process_button_thread, args=(new_file_name,))
        # Start the thread
        thread.start()
