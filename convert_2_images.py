...
def plot_mel_spectrogram(y: np.ndarray, file_name: str) -> None:
    S = librosa.feature.melspectrogram(
        y=np.array(y), sr=16000, n_fft=256, hop_length=64
    )
    S_dB = librosa.power_to_db(S, ref=np.max)

    plt.figure(figsize=(6, 4))
    librosa.display.specshow(S_dB, sr=16000, cmap="viridis")
    plt.tight_layout(pad=0)
    plt.savefig("./mel/mel_" + file_name + ".png")


def plot_rangeDoppler(y: np.ndarray, file_name: str) -> None:
    plt.figure(figsize=(6, 4))
    ax = plt.axes()
    im = ax.imshow(np.array(y), cmap='viridis', interpolation='nearest', aspect='auto')
    ax.set_axis_off()
    plt.tight_layout(pad=0)
    plt.savefig("./rD/rangeDoppler_" + file_name + ".png")

if __name__ == "__main__":
    train_set = DataLoader(
        CombinedDataSet(
            fileDir="/home/gevindu/model_final/FYP-Drone-Classification/Data Collection - Collection state - New.csv",
            verbose=False,
        ),
        batch_size=128,
        shuffle=True,
    ...
    for batch_num, (doppler, audio, det_label, cls_label) in enumerate(train_set):
        for index in range(doppler.shape[0]):
            file_name = f"data_{batch_num*doppler.shape[0]+index}_det_{int(det_label[index])}_cls_{int(cls_label[index])}"
            plot_mel_spectrogram(audio[index], file_name)
            plot_rangeDoppler(doppler[index], file_name)